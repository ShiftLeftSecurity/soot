...
Composite jopjop_ctlChild = jopjop_ctlCreate(getPageContainer());
...
Composite dbdb_force_recompileChild = dbdb_force_recompileCreate(getPageContainer());
	...
	private void initializeEnableGroups(){
		...
		makeNewEnableGroup("jop", "jop.ctl");
		
		
		addToEnableGroup("jop", "jop.ctl", getjopjop_ctlenabled_widget(), "enabled");
		
		getjopjop_ctlenabled_widget().getButton().addSelectionListener(this);
	...
	private boolean createNewConfig() {
		...
		stringRes = getProcessing_Optionstr_widget().getText().getText();
		
		defStringRes = "";
		

	        if ( (!(stringRes.equals(defStringRes))) && (stringRes != null) && (stringRes.length() != 0)) {
			getConfig().put(getProcessing_Optionstr_widget().getAlias(), stringRes);
		}
		...
		boolRes = getjopjop_ctlenabled_widget().getButton().getSelection();
		...
		if (boolRes != defBoolRes) {
			getConfig().put(getjopjop_ctlenabled_widget().getAlias(), new Boolean(boolRes));
		}
	...
	protected SootOption getInitialInput() {
		...
		//Phase Options
			...
			SootOption jop_jop_ctl_branch = new SootOption("CTL-FV based code transformer", "jopjop_ctl");
			subParent.addChild(jop_jop_ctl_branch);


			

			
			subSectParent = jop_jop_ctl_branch;
	...
	private StringOptionWidget Processing_Optionstr_widget;
	
	private void setProcessing_Optionstr_widget(StringOptionWidget widget) {
		Processing_Optionstr_widget = widget;
	}
	
	public StringOptionWidget getProcessing_Optionstr_widget() {
		return Processing_Optionstr_widget;
	}
	...
	private BooleanOptionWidget jopjop_ctlenabled_widget;
	
	private void setjopjop_ctlenabled_widget(BooleanOptionWidget widget) {
		jopjop_ctlenabled_widget = widget;
	}
	
	public BooleanOptionWidget getjopjop_ctlenabled_widget() {
		return jopjop_ctlenabled_widget;
	}	
	...
	private Composite Processing_OptionsCreate(Composite parent) {
		...
		setProcessing_Optionstr_widget(new StringOptionWidget(editGroupProcessing_Options, SWT.NONE, new OptionData("Transformations",  "", "","tr", "\nReads CTL-FV based transformations from the file TRANS_FILE. ", defaultString)));
	...
	private Composite jopjop_ctlCreate(Composite parent) {
		String defKey;
		String defaultString;
		boolean defaultBool = false;
	    String defaultArray;
       
		Group editGroupjopjop_ctl = new Group(parent, SWT.NONE);
		GridLayout layout = new GridLayout();
		editGroupjopjop_ctl.setLayout(layout);
	
	 	editGroupjopjop_ctl.setText("CTL-FV based code transformer");
	 	
		editGroupjopjop_ctl.setData("id", "jopjop_ctl");
		
		String descjopjop_ctl = "CTL-FV based code transformer";	
		if (descjopjop_ctl.length() > 0) {
			Label descLabeljopjop_ctl = new Label(editGroupjopjop_ctl, SWT.WRAP);
			descLabeljopjop_ctl.setText(descjopjop_ctl);
		}
		OptionData [] data;	
		...
		setjopjop_ctlenabled_widget(new BooleanOptionWidget(editGroupjopjop_ctl, SWT.NONE, new OptionData("Enabled", "p", "jop.ctl","enabled", "\n", defaultBool)));
		
		

		
		return editGroupjopjop_ctl;
	}
