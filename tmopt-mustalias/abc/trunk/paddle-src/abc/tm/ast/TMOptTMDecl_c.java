...
/**
 * A customized tracematch declaration which instantiates a customized instance of {@link TraceMatch},
 * which we need for added functionality.
 *
 * @author Eric Bodden
 */
public class TMOptTMDecl_c extends TMDecl_c {

	public TMOptTMDecl_c(Position pos, Position body_pos, TMModsAndType mods_and_type, String tracematch_name, List formals, List throwTypes, List symbols, List frequent_symbols, Regex regex, Block body) {
		super(pos, body_pos, mods_and_type, tracematch_name, formals, throwTypes,
				symbols, frequent_symbols, regex, body);
	}
	
	/**
	 * This method is just copied over from the super class.
	 * The only difference is that we create another advice info.
	 */
	public void update(GlobalAspectInfo gai, Aspect current_aspect)
	{
	    ...
	    // create TraceMatch
	    TraceMatch tm =
	        new TMOptTraceMatch(tracematch_name, tm_formals, body_formals,
