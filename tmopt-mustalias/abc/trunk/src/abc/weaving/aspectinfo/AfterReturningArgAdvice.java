...
public class AfterReturningArgAdvice extends AfterReturningAdvice {
    ...
    public Residue matchesAt(MatchingContext mc,ShadowMatch sm,AbstractAdviceDecl ad) {
	if(super.matchesAt(mc,sm,ad)==null) return null;
	ContextValue cv=sm.getReturningContextValue();
	weavingVar = mc.getWeavingEnv().getWeavingVar(var, mc);
	return Bind.construct
	    (cv,mc.getWeavingEnv().getAbcType(var).getSootType(),weavingVar);
    }
