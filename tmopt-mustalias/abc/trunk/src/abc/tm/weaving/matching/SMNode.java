...
public class SMNode implements State {
    ...
    protected Set/*<SMEdge>*/ incoming = new IdentityHashSet();
    protected Set/*<SMEdge>*/ outgoing = new IdentityHashSet();
     ...
     */
    public void fillInClosure(Set result, final boolean epsilonOnly, boolean forward) {
        fillInClosure(result, forward, new SMEdgePredicate() {

			public boolean accept(SMEdge e) {
				return epsilonOnly && e.getLabel() != null;
			}
    ...
    /**
     * Same as {@link #fillInClosure(Set, boolean, boolean)} but decides over a generic predicate
     * what edges to skip.
     * @param pred a predicate which should return <code>true</code> for an edge that shall be <i>skipped</i>
     */
    public void fillInClosure(Set result, boolean forward, SMEdgePredicate pred) {
        SMEdge cur;
        SMNode next;
        result.add(this);
        Iterator it;
        ...
        while(it.hasNext()) {
            cur = (SMEdge)it.next();
            if(!pred.accept(cur)) { // Epsilon transition
                ...
                if(!result.contains(next)) { // we haven't visited that node
                    next.fillInClosure(result, forward, pred);
                }
            }
        }
    }
    ...
    /**
     * Fills in the "epsilon boundary" of this state,
     * which are all final states that are epsilon-reachable from this states
     * plus all nonfinal states that are epsilon-reachable and have an outgoing
     * non-epsilon transition.
     * @param result the result will be passed in here
     */
    protected void fillInEpsilonBoundary(Set result) {
    	fillInEpsilonBoundary(result, new HashSet());
    }
    
    /**
     * @see #fillInEpsilonBoundary(Set)
     */
    protected void fillInEpsilonBoundary(Set result, Set visited) {
        SMEdge outEdge;
        SMNode next;
        Iterator it;
        visited.add(this);
        ...
        while(it.hasNext()) {
            outEdge = (SMEdge)it.next();
            if(outEdge.getLabel()==null) { // Epsilon transition
                next = outEdge.getTarget(); 
                if(!visited.contains(next)) { // we haven't visited that node
                    next.fillInEpsilonBoundary(result,visited);
                }
            } else {
            	result.add(this);
            }
     ...
     * <b>This does not take skip edges into account!</b>
    ...
    public boolean hasInEdgeWithLabel(String label) {
    	...
    	while(it.hasNext()) {
    		edge = (SMEdge) it.next();
    		if(!edge.isSkipEdge() && (edge.getLabel() == label || (label != null && label.equals(edge.getLabel())))) {
    			return true;
    		}
    	}
    	return false;
    ...
    /**
     * Determines whether there exists an edge to a node with equal content to the one that is passed in.
     * @param to The target node
     * @param prototypeEdge edge to compare against
     * @return
     */
    public boolean hasEqualEdgeTo(SMNode to, SMEdge prototypeEdge) {
        ...
        while(it.hasNext()) {
            edge = (SMEdge)it.next();
            if(edge.getTarget() == to && edge.equals(prototypeEdge)) {
            	result = true;
            	break;
            }
        }
     ...
     * @return
     * @see #hasAnyEdgeTo(SMNode, String)
     * @deprecated use {@link #hasEqualEdgeTo(SMNode, SMEdge)} instead; because
     * the label does not always determine the full nature of the edge (e.g.
     * for {@link SMVariableEdge}s this is the case)
     */
    public boolean hasEdgeTo(SMNode to, String label) {
        ...
        while(it.hasNext()) {
            edge = (SMEdge)it.next();
            if(!edge.isSkipEdge() && (edge.getTarget() == to && 
    ...
    public void removeInEdge(SMEdge edge) {
        if(!incoming.remove(edge)) {
        	new RuntimeException("tried to remove an in-edge which was not present: "+edge).printStackTrace();
        }
    }
    
    public void removeOutEdge(SMEdge edge) {
        if(!outgoing.remove(edge)) {
        	new RuntimeException("tried to remove an out-edge which was not present: "+edge).printStackTrace();
        }
    }
