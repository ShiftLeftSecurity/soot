...
public class IntraProceduralTMFlowAnalysis extends ForwardFlowAnalysis implements TMFlowAnalysis {
	...
	private final LocalMustAliasAnalysis lmaa;

	private final Map<Local, Stmt> tmLocalsToDefStatements;
	 ...
	 */
	public IntraProceduralTMFlowAnalysis(TraceMatch tm, UnitGraph ug, Disjunct prototype, LocalMustAliasAnalysis localMustAliasAnalysis, Map<Local, Stmt> tmLocalsToDefStatements, InitKind initializationKind) {
		super(ug);
	...
	protected void flowThrough(Object in, Object d, Object out) {
		...
		for (SymbolShadow shadow : matchesForThisTracematch) {
			if(shadow.isEnabled()) {				
				...
				Configuration newConfig = inConfig.doTransition(shadow,lmaa,tmLocalsToDefStatements);
