...
public class Configuration implements Cloneable {
	 ...
	 */
	public Configuration doTransition(SymbolShadow shadow, LocalMustAliasAnalysis lmaa, Map<Local, Stmt> tmLocalsToDefStatements) {
		...
		final Map<String,Local> varToLocal = shadow.getTmFormalToAdviceLocal();
		final Map<String,InstanceKey> bindings = new HashMap<String, InstanceKey>();
		for (Entry<String,Local> entry : varToLocal.entrySet()) {
			String var = entry.getKey();
			Local l = entry.getValue();
			InstanceKey reachingInstanceKey = lmaa.reachingInstanceKey(l, tmLocalsToDefStatements.get(l));
			bindings.put(var, reachingInstanceKey);
		}
