...
public class LocalNotMayAliasAnalysis extends ForwardFlowAnalysis
{
    ...
    public boolean mayPointTo(Local l, Stmt s, InstanceKey k) {
    	if(k.isUnknown())
    		return false;
    	else {
    		InstanceKey.ExprBasedInstanceKey key = (InstanceKey.ExprBasedInstanceKey) k;
    		Expr e = key.getExpr();
	    	Set ln = (Set) ((HashMap)getFlowBefore(s)).get(l);
	    	if(ln.contains(UNKNOWN))
	    		return true;
	    	else {
	    		return !ln.contains(e);
	    	}
    	}
    }
}
