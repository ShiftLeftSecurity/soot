...
//    }
    ...
    public InstanceKey reachingInstanceKey(Local l, Stmt s) {
    	Object ln = ((HashMap)getFlowBefore(s)).get(l);
    	if(ln==null) {
    		throw new RuntimeException("no result known for "+l+" at "+s);
    	} else if(ln == UNKNOWN)
    		return InstanceKey.UNKNOWN;
    	else {
    		Expr e = (Expr) ln;
    		return new InstanceKey.ExprBasedInstanceKey(e);
    	}
    }
