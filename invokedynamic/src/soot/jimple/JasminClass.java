...
public class JasminClass extends AbstractJasminClass {
	...
	protected void assignColorsToLocals(Body body) {
		...
		FastColorer.assignColorsToLocals(body, localToGroup, localToColor,
				groupToColorCount);
	...
	protected void emitMethodBody(SootMethod method)// , Map options)
	{
		...
		// boolean disablePeephole = PhaseOptions.getBoolean(options,
		// "no-peephole");
		...
		// Determine where the locals go
		{
			...
			Map<GroupIntPair, Integer> groupColorPairToSlot = new HashMap<GroupIntPair, Integer>(
			...
			// Handle identity statements
			{
				...
				while (stmtIt.hasNext()) {
					...
					if (s instanceof IdentityStmt
						...
						IdentityRef identity = (IdentityRef) ((IdentityStmt) s)
								.getRightOp();
						...
						// so that all locals of the same color can be pointed
						// here too
						{

							GroupIntPair pair = new GroupIntPair(
									localToGroup.get(l), localToColor.get(l)
											.intValue());
			...
			// Assign the rest of the locals
			{
				...
				while (localIt.hasNext()) {
					...
					if (!assignedLocals.contains(local)) {
						GroupIntPair pair = new GroupIntPair(
								localToGroup.get(local), localToColor
										.get(local).intValue());

						int slot;

						if (groupColorPairToSlot.containsKey(pair)) {
							// This local should share the same slot as the
							// previous local with
		...
		// Emit code in one pass
		{
			...
			while (codeIt.hasNext()) {
				...
				if (subroutineToReturnAddressSlot.containsKey(s)) {
					...
					modifyStackHeight(1); // simulate the pushing of address
											// onto the stack by the jsr
					...
					// emit("astore " + ( ( Integer )
					// subroutineToReturnAddressSlot.get( s ) ).intValue() );
				...
				do {
					...
					// a: <lvalue> = <rvalue>; <rvalue> = <rvalue> +/- 1;
					// use(<lvalue>);
					// b: <lvalue> = <rvalue>; <rvalue> = <lvalue> +/- 1;
					// use(<lvalue>);
					...
					if (addexp.getOp1().equivTo(lvalue)) {
						if (lu.getUsesOf(stmt).size() != 2
								|| ld.getDefsOfAt((Local) lvalue, nextStmt)
										.size() != 1
								|| ld.getDefsOfAt((Local) lvalue, nextNextStmt)
										.size() != 1)
							break;
						plusPlusState = 0;
					} else {
					...
					if (currentStackHeight != 0)
						throw new RuntimeException("Stack has height "
								+ currentStackHeight
								+ " after execution of stmt: " + s);
					contFlag = true;
					codeIt.next();
					codeIt.next();
				} while (false);
	...
	void emitAssignStmt(AssignStmt stmt) {
		...
		if (lvalue instanceof Local
				&& (rvalue instanceof AddExpr || rvalue instanceof SubExpr)) {
			...
			if (l.getType().equals(IntType.v())) {
				...
				if (op1 == l && op2 instanceof IntConstant) {
					...
					isValidCase = true;
				} else if (expr instanceof AddExpr && op2 == l
						&& op1 instanceof IntConstant) {
					// Note expr can't be a SubExpr because that would be x = 3
					// - x
		...
		lvalue.apply(new AbstractJimpleValueSwitch() {
			...
			public void caseInstanceFieldRef(InstanceFieldRef v) {
				...
				emit("putfield "
						...
						+ "/" + v.getFieldRef().name() + " "
						+ jasminDescriptorOf(v.getFieldRef().type()), -1
			...
			public void caseStaticFieldRef(StaticFieldRef v) {
				...
				emit("putstatic " + slashify(field.declaringClass().getName())
						+ "/" + field.name() + " "
						+ jasminDescriptorOf(field.type()), -sizeOfType(v
						.getFieldRef().type()));
			}
		});
	}
		...
		cond.apply(new AbstractJimpleValueSwitch() {
			...
			public void caseNeExpr(NeExpr expr) {
				op1.getType().apply(new TypeSwitch() {
					...
					public void defaultCase(Type t) {
						throw new RuntimeException("invalid type for NeExpr: "
								+ t);
					}
				});
			}
		...
		stmt.apply(new AbstractStmtSwitch() {
			...
			public void caseIdentityStmt(IdentityStmt s) {
				if (s.getRightOp() instanceof CaughtExceptionRef
					...
					modifyStackHeight(1); // simulate the pushing of the
											// exception onto the
											// stack by the jvm
			...
			public void caseInvokeStmt(InvokeStmt s) {
				...
				Type returnType = ((InvokeExpr) s.getInvokeExpr())
			...
			public void caseGotoStmt(GotoStmt s) {
				if (isNextGotoAJsr) {
					...
					subroutineToReturnAddressSlot.put(s.getTarget(),
							new Integer(returnAddressSlot));
				} else
			...
			public void caseTableSwitchStmt(TableSwitchStmt s) {
				emitValue(s.getKey());
				emit("tableswitch " + s.getLowIndex() + " ; high = "
						+ s.getHighIndex(), -1);
		...
		value.apply(new AbstractGrimpValueSwitch() {
			...
			public void caseCastExpr(final CastExpr v) {
				...
				if (toType instanceof RefType)
					...
					fromType.apply(new TypeSwitch() {
						public void defaultCase(Type ty) {
							throw new RuntimeException("invalid fromType "
									+ fromType);
						}
			...
			public void caseInterfaceInvokeExpr(InterfaceInvokeExpr v) {
				...
				emit("invokeinterface "
						...
						+ (argCountOf(m) + 1), -(argCountOf(m) + 1)
						+ sizeOfType(m.returnType()));
			}
			...
			public void caseNegExpr(final NegExpr v) {
				...
				v.getType().apply(new TypeSwitch() {
					...
					public void defaultCase(Type t) {
						throw new RuntimeException(
								"Invalid argument type for neg: " + t + ": "
										+ v);
					}
				});
			...
			public void caseNewMultiArrayExpr(NewMultiArrayExpr v) {
				...
				emit("multianewarray " + jasminDescriptorOf(v.getBaseType())
			...
			public void caseUshrExpr(UshrExpr v) {
				...
				v.getType().apply(new TypeSwitch() {
					...
					public void defaultCase(Type t) {
						throw new RuntimeException(
								"Invalid argument type for ushr");
					}
				});
			}
