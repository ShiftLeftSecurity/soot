   ...
   static JimpleBody jimplify( SootMethod method ){
      ...
      try{
         ...
         if ( ! CLGB )
         {
            ...
            cgnumstmts = cgnumstmts + jimpleBody.getUnits().size();
            if ( ! ( isJava || isSun || isSunw || isJavax || isOrg || isCom ) )
            {
               benchjimplifiedNum++;
               cgbenchnumstmts = cgbenchnumstmts + jimpleBody.getUnits().size();
               totnumbenchmethods++;
               ...
               benchnumstmts = benchnumstmts + jimpleBody.getUnits().size();
            }
            ...
            sizeHT.put ( method.getSignature(), new Integer ( jimpleBody.getUnits().size() ) );
            numstmts = numstmts + jimpleBody.getUnits().size();
         }
