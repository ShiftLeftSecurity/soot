            ...
            if ( ( ! meth.getName().equals ( "<clinit>" ) ) && ( ! Modifier.isNative( meth.getModifiers() ) ) )
            {
               if ( allowedToChange ( meth.getDeclaringClass().getName() ) )
               {
                  ...
                  try {
                     ...
                     // if ( body.getUnits().size() < 200 )

                     // if ( stmtsAtSite < 60  )

                     if ( ( stmtsAtSite < 20 ) && ( ( body.getUnits().size() < ( 8*origSize ) ) && ( body.getUnits().size() < 10000 ) ) )
                     // if ( stmtsAtSite < 10  )
                     {
                        numstmts = 0;
                        looking = true;
                           ...
                           if ( ( cs.getMethods().size() == 1 ) && ( invokeflag || assignflag ) )
                           {
                              ...
                              if ( ! ( /* incorrectlyjimplified.contains ( me.getDeclaringClass().getName() ) || */ me.getName().equals("<init>") ) )
                              {
                                 if ( satisfiesCriteria ( me, currInvokeExpr ) )
                                 {
                                    ...
                                    stmtsAtSite = stmtsAtSite + mebody.getUnits().size();
                                    ...
                                    if ( ! staticInvoked )
                                    {
                                       ...
                                       insertNullCheck ( baseval, dummynullcheck, body.getUnits(), stmtIter );
                                    }
                                    ...
                                    InlineMethod ( mebody.getUnits() , body.getUnits(), stmtIter );
                                    gotoEliminate ( body );
         ...
         try {
            ...
            while ( methit.hasNext() )
            {
               ...
               try {
                  ...
                  aftersize = aftersize + changedjb.getUnits().size();
                  ...
                  beforesize = beforesize + changedjb.getUnits().size();
                  aftersize = aftersize + changedjb.getUnits().size();
...
*/
   ...
   boolean satisfiesCriteria ( SootMethod m, InvokeExpr ie ) {
      try {
         ...
         if ( ( Jimplifier.getJimpleBody ( m ).getUnits().size() > averageSize ) /* && unimportantmethod */ )
         {
            criteria2++;
            return false;
         }
   ...
   Stmt getCorrectStmt ( Stmt stmt ) {
      stmt.apply( new AbstractStmtSwitch(){
         public void caseIdentityStmt ( IdentityStmt s ) {
            ...
            correctstmt = jimple.newIdentityStmt ( getCorrectValue ( leftop ), jimple.newCaughtExceptionRef( body ) );
         }
   ...
   void FixupTraps () {
      ...
      Iterator trapiterator = mebody.getTraps().iterator();
      int trapindex = 0;
