...
public final class SharedPointsToSet extends PointsToSetInternal {
    ...
    private final boolean nativeAddAll( SharedPointsToSet other, SharedPointsToSet exclude ) {
        boolean ret = false;
        BitVector mask = null;
        AbsTypeManager typeManager = SparkScene.v().tm;
    ...
    public final boolean forall( P2SetVisitor v ) {
       if( bits == null ) {
            ...
            for( BitSetIterator it = bits.iterator(); it.hasNext(); ) {
                v.visit( (Node) SparkNumberers.v().allocNodeNumberer().get( it.next() ) );
            }
        }
    ...
    public final boolean add( Node n ) {
        if( SparkScene.v().tm.castNeverFails( n.getType(), type ) ) {
            return fastAdd( n );
        }
        return false;
    }
    ...
    protected final void convertToBits() {
        if( bits != null ) return;
        bits = new SharedBitSet( SparkNumberers.v().allocNodeNumberer().size() );
