...
public class SparkNativeHelper extends NativeHelper {
    protected void assignImpl(ReferenceVariable lhs, ReferenceVariable rhs) {
        SparkScene.v().nodeFactory().addEdge( (Node) rhs, (Node) lhs );
    }
    protected void assignObjectToImpl(ReferenceVariable lhs, AbstractObject obj) {
	AllocNode objNode = SparkScene.v().nodeManager().makeAllocNode( 
		new Pair( "AbstractObject", obj.getType() ),
		 obj.getType(), null );

        VarNode var;
        if( lhs instanceof FieldRefNode ) {
	    var = SparkScene.v().nodeManager().makeGlobalVarNode( objNode, objNode.getType() );
            SparkScene.v().nodeFactory().addEdge( (Node) lhs, var );
        } else {
            var = (VarNode) lhs;
        }
        SparkScene.v().nodeFactory().addEdge( objNode, var );
    }
	...
	if( base instanceof VarNode ) {
	    ...
	    l = SparkScene.v().nodeManager().makeGlobalVarNode( b, b.getType() );
	    SparkScene.v().nodeFactory().addEdge( b, l );
	}
        return SparkScene.v().nodeManager().makeFieldRefNode( l, ArrayElement.v() );
    }
    ...
    protected ReferenceVariable newInstanceOfImpl(ReferenceVariable cls) {
        return SparkScene.v().nodeFactory().caseNewInstance( (VarNode) cls );
    }
	...
	return SparkScene.v().nodeManager().makeGlobalVarNode( f, f.getType() );
    }
    protected ReferenceVariable tempFieldImpl(String fieldsig) {
	return SparkScene.v().nodeManager().makeGlobalVarNode( new Pair( "tempField", fieldsig ),
            RefType.v( "java.lang.Object" ) );
    }
    protected ReferenceVariable tempVariableImpl() {
	return SparkScene.v().nodeManager().makeGlobalVarNode( new Pair( "TempVar", new Integer( ++G.v().SparkNativeHelper_tempVar ) ),
		RefType.v( "java.lang.Object" ) );
    }
}
