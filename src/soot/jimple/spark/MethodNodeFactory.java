...
public abstract class MethodNodeFactory extends AbstractJimpleValueSwitch {
    ...
    public MethodNodeFactory() {
        gnf = SparkScene.v().nodeFactory();
        nm = SparkScene.v().nodeManager();
    }
	...
	if( SparkScene.v().options().field_based() || SparkScene.v().options().vta() ) {
    ...
    final public void caseNewExpr( NewExpr ne ) {
        if( SparkScene.v().options().merge_stringbuffer() 
    ...
    final public void caseStringConstant( StringConstant sc ) {
        ...
        if( SparkScene.v().options().string_constants()
