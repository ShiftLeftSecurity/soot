...
public final class BDDVirtualCalls {
    ...
    public void addTypes(final jedd.Relation newTypes) {
        this.hier.update();
        newTypes.eqMinus(jedd.Jedd.v().project(this.answer, new jedd.PhysicalDomain[] { V2.v() }));
        final jedd.Relation toResolve =
          new jedd.Relation(new jedd.Domain[] { subt.v(), signature.v(), supt.v() },
                            new jedd.PhysicalDomain[] { T1.v(), H1.v(), T2.v() },
                            jedd.Jedd.v().join(jedd.Jedd.v().read(newTypes),
                                               jedd.Jedd.v().replace(newTypes,
        ...
        do  {
            final jedd.Relation resolved =
              new jedd.Relation(new jedd.Domain[] { subt.v(), signature.v(), supt.v(), method.v() },
    ...
    public jedd.Relation answer() {
        return new jedd.Relation(new jedd.Domain[] { signature.v(), method.v(), type.v() },
                                 new jedd.PhysicalDomain[] { H1.v(), V2.v(), T1.v() },
                                 this.answer);
    }
    
    private final jedd.Relation declaresMethod =
      new jedd.Relation(new jedd.Domain[] { type.v(), signature.v(), method.v() },
        ...
        for (Iterator clIt = Scene.v().getClasses().iterator(); clIt.hasNext(); ) {
            ...
            for (Iterator mIt = cl.getMethods().iterator(); mIt.hasNext(); ) {
                ...
                this.declaresMethod.eqUnion(jedd.Jedd.v().literal(new Object[] { m.getDeclaringClass().getType(), m.getNumberedSubSignature(), m },
                                                                  new jedd.Domain[] { type.v(), signature.v(), method.v() },
                                                                  new jedd.PhysicalDomain[] { T2.v(), H1.v(), V2.v() }));
            }
        }
    }
